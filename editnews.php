<?php 
	...
	if (isset($_GET["deletenews"])) {
		if (isset($_SESSION["username"])) {
			$news_id = $_GET["deletenews"];
			$sql = "SELECT * FROM news WHERE id=$news_id";
			$res = $conn -> query($sql);
			$row = $res -> fetch_assoc();
			if ($_SESSION["user_id"] == $row["user_id"] || $_SESSION["role"] == 1) {
				$sql = "DELETE FROM comment WHERE id=$news_id";
				$conn -> query($sql);
	...
	if (isset($_POST["title"])) {
		...
		if ($content == "") $_SESSION["err"] = "<script>alert('Nội dung không để trống!!!');</script>";
			...
			$sql = "UPDATE news SET title='{$title}', summary='{$summary}', subcat_id=$subcat_id, image='{$image}', content='{$content}' WHERE id=$news_id";
			$conn -> query($sql);
   	...
   	<div class="row ml-5 mr-5 mt-5">
	   	<div class="col-8 pr-0">
	   		<div class="bg-white shadow rounded-lg p-4 mb-5 ml-auto mr-auto">
	   			<form action="<?php echo "editnews.php?editnews=$news_id"; ?>" method="POST">
	         		<div class="text-center">
	            		<img src="<?php echo $row['image']; ?>" id="showimg" width="100%" class="border border-info rounded"> <br>
	            		<input type="file" id="imgInp" style="display: none;">
	            		<input type="text" name="image" id="image" value="<?php echo $row['image']; ?>" hidden>
	            		<button type="button" class="btn btn-info mt-2" data-toggle="modal" data-target="#upload"><i class="fas fa-upload"></i> Chọn ảnh</button> 
	         		</div>
	         		Tiêu đề:<input type="text" class="form-control" name="title" value="<?php echo $row["title"]; ?>" maxlength="150" required>
	         		Chuyên mục: 
...
<?php  
	...
	while ($rowsub = $ressub -> fetch_assoc()) {
		if ($row["subcat_id"] == $rowsub["id"]) {
			echo "<option value='".$rowsub['id']."' selected>".$rowsub['subcategory']." (".$rowsub['category'].")</option>";
		} else echo "<option value='".$rowsub['id']."'>".$rowsub['subcategory']." (".$rowsub['category'].")</option>"; 
	}
?>
	         		</select>  
	         		Tóm tắt:<textarea class="form-control" name="summary" rows="3" maxlength="500" required><?php echo $row["summary"]; ?></textarea>
	         		Nội dung:
	         		<textarea class="form-control" id="content" name="content" rows="4"><?php echo $row["content"]; ?></textarea>
	         		<input type="submit" value="Cập nhật" class="btn btn-info btn-block btn-lg">
	        		</form>
		      </div>
	   	</div>
	   	<div class="col">
	   	    <a href="addnews.php" class="btn btn-outline-info w-100 mb-1"><h2 class="mb-0">ĐĂNG BÀI <i class="fas fa-feather-alt"></i></h2></a>
	   		<a href="<?php echo 'news.php?news_id='.$news_id; ?>" class="btn btn-info w-100 mb-1"><h2 class="mb-0">QUAY VỀ BÀI VIẾT</h2></a>
